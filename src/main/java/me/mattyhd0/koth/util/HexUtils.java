...
public final class HexUtils {
    ...
    private static final Pattern LEGACY_HEX = Pattern.compile("§[0-9A-Fa-f]");
    ...
    private static Method colorOf, colorOfStr;

    static {
        try {
            colorOf = ChatColor.class.getMethod("of", java.awt.Color.class);
            colorOfStr = ChatColor.class.getMethod("of", String.class);
    ...
    public static Color decodeHEX(String value) {
        if (StringUtil.isEmpty(value)) return new Color(0, 0, 0);

        return Color.decode(value.indexOf('#') == -1 ? '#' + value : value);
    }

    public static Color getMainColor(String... hexColors) {
        int totalRed = 0;
        int totalGreen = 0;
        int totalBlue = 0;

        for (String hexColor : hexColors) {
            if (StringUtil.isEmpty(hexColor)) continue;

            Color color = decodeHEX(hexColor);
            totalRed += color.getRed();
            totalGreen += color.getGreen();
            totalBlue += color.getBlue();
        }

        int averageRed = totalRed / hexColors.length;
        int averageGreen = totalGreen / hexColors.length;
        int averageBlue = totalBlue / hexColors.length;
    ...
    public static String[] extractHexesFromLegacy(String value) {
        if (StringUtil.isEmpty(value)) return new String[0];

        Matcher matcher = LEGACY_HEX.matcher(value);
        StringBuilder hexCodes = new StringBuilder();

        while (matcher.find()) {
            hexCodes.append(matcher.group().substring(1));
        }

        return hexCodes.toString().replaceAll("(?<=\\G.{6})", "#").split("#");
    }

    public static ChatColor of(String color) {
        if (colorOfStr != null) try {
            return (ChatColor) colorOfStr.invoke(null, color);
        } catch (Throwable e) {
    ...
    public static ChatColor of(java.awt.Color color) {
        if (colorOf != null) try {
            return (ChatColor) colorOf.invoke(null, color);
        } catch (Throwable e) {
            throw new RuntimeException(e);
        }
